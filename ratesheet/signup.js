 ...
 * See api/public.php's header for Michael's verbatim request and the full
 * server-side flow.
 *
 * One screen: info + address + Card/ACH PREFERENCE (no card or bank
 * numbers collected here at all -- see PAYMENT DATA below) + terms +
 * signature, submitted in a single api/public.php?action=submit call.
 * Which screen shows is still driven by the row's own `status` (from
 * ?action=context), so a page reload resumes in the right place:
 *   'pending' | 'failed'  -> the signup form
 *   'submitted'           -> "Already Submitted"
 *
 * PAYMENT DATA: per Michael (2026-09-18), Payment Method here is a plain
 * PREFERENCE only (Card vs. ACH) -- no card number, no bank routing/
 * account number, nothing self-service. Invoicing adds the real payment
 * method directly in Alternative Payments' own dashboard after this form
 * is submitted (see api/public.php's header for the full reasoning,
 * including the PCI-DSS decision from 2026-09-17 that this app must never
 * collect raw card/bank numbers). Do not add a card/bank number field to
 * this form without revisiting that decision explicitly.
 *
 * LEGAL TEXT: RATESHEET_LEGAL_TEXT and CHECKBOX_TEXT below are both
 * Michael's real, verbatim wording (chat, 2026-09-17). Keep
 * RATESHEET_LEGAL_TEXT identical to api/_util.php's ratesheet_legal_text()
 * if either is ever updated.
 */
...
(function () {
  ...
  // Keep in sync with api/_util.php's ratesheet_legal_text().
  ...
  // reset the signature pad canvas on every validation error -- a real,
  // previously-hit bug. Full render() is still used for the few
  // transitions that legitimately need to swap the whole screen (initial
  // load, final "Thank You" screen).
  ...
  function submitForm() {
    ...
    apiPost('api/public.php?action=submit&t=' + encodeURIComponent(token), body).then(function (r) {
      ...
      if (r.data && r.data.ok) {
        ...
        showFormError((r.data && r.data.error) || 'Something went wrong submitting your information. Please try again, or contact CodeBlue Technology.');
        setSubmitButtonState();
      }
    }).catch(function () {
      state.submitting = false;
  ...
  function letterheadHtml() {
    return '<div class="letterhead"><div class="name">CodeBlue Technology</div><div class="sub">Customer Rate Sheet Sign Up</div></div>';
  }
  ...
  function render() {
    ...
    if (state.submitted) {
      root.innerHTML = '<div class="wrap"><div class="card success-card"><div class="big">✅</div><h2>Thank You!</h2>' +
        '<p style="color:#5A6472;">Your rate sheet sign up has been received. A CodeBlue Technology team member will follow up with you shortly to finish adding your payment method on file.</p></div>' +
        '<div class="footer-contact">CodeBlue Technology &nbsp;|&nbsp; (804) 521-7660 &nbsp;|&nbsp; Service@codebluetechnology.com</div></div>';
      return;
    }
    // 'pending' or 'failed' (retry) -- and any legacy 'awaiting_payment'
    // row (superseded flow, see db.php) -- all just show the single form.
    renderForm();
  }
